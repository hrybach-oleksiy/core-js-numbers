...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const point1 = Math.sqrt(x1 * x1 + y1 * y1);
  const point2 = Math.sqrt(x2 * x2 + y2 * y2);

  return Math.acos(dotSum / (point1 * point2));
}
...
function getSumOfDigits(num) {
  ...
  const numberArr = numberToString.split('');
  return numberArr.reduce((acc, curr) => acc + Number(curr), 0);
}
